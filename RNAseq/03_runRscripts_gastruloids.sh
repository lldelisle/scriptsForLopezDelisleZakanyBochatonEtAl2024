...
#SBATCH --job-name RNAseq_R_gastruloids # Job name that appear in squeue as well as in output and error text files
...
filePathForSamplesPlan="$PWD/samplesplan_Gastruloids.txt"
...
" > ${configFile}
  ...
  cp ${dirPathWithResults}/mergedTables/AllHTSeqCounts.txt ${dirPathWithResults}/toGEO/
  mkdir -p ${dirPathWithConfigFiles}/outputs
  gzip -c ${dirPathWithResults}/mergedTables/AllCufflinks_Simplified.txt > ${dirPathWithConfigFiles}/outputs/AllCufflinks_Simplified.txt.gz
  gzip -c ${dirPathWithResults}/mergedTables/AllHTSeqCounts.txt > ${dirPathWithConfigFiles}/outputs/AllHTSeqCounts.txt.gz
fi
